package ru.job4j.algo.exam.cicletree;


import ru.job4j.algo.exam.cicletree.Node;
...
public class TreeUtils<T> {
    ...
    private List<Node<T>> findAllNodes(Node<T> root) {
        SimpleQueue<Node<T>> queue = new SimpleQueue<>();
    ...
    /* Время здесь квадратичное */
    public Optional<Node<T>> checkerCycle(Node<T> root) {
        Optional<Node<T>> result = Optional.empty();
        Set<Node<T>> tempSet = new HashSet<>();
        List<Node<T>> tempList = findAllNodes(root);
        for (Node oneNode : tempList) {
            if (!tempSet.add(oneNode)) {
                for (Node oneNodeInSet : tempSet) {
                    if (Objects.equals(oneNode, oneNodeInSet)) {
                        return Optional.of(oneNode);
                    }
                }
            }
        }
